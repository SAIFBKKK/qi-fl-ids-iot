"""
train_final_mlp_deeper_balanced_v3.py
=====================================

Deeper final centralized MLP training script for the balanced CICIoT2023 dataset.

Architecture:
    32 -> 128 -> 64 -> 34

Changes vs previous final script:
- deeper MLP
- no WeightedRandomSampler
- no class weights
- pure balanced training
- scheduler + early stopping kept
"""
...
OUTPUT_DIR = Path(r"E:\dataset\processed_merged_full\final_mlp_deeper_balanced_v3_results")
...
class DeeperFinalMLP(nn.Module):
...
def main():
    ...
    print("Architecture:", "32 -> 128 -> 64 -> 34", flush=True)
    print("Loss:", "CrossEntropyLoss", flush=True)
    print("Sampler:", "None", flush=True)
    ...
    model = DeeperFinalMLP(input_dim=len(feature_cols), num_classes=len(label_to_id)).to(DEVICE)
    ...
    if SAVE_PLOTS:
        save_confusion_matrix(
            ...
            "Confusion Matrix — Final Deeper MLP (32 -> 128 -> 64 -> 34)"
        )
    ...
    summary = {
        ...
        "model": "Final Deeper PyTorch MLP",
