"""
train_hierarchical_level3_submodel.py
====================================
"""
...
ROOT_OUT=Path(r"E:\dataset\processed_merged_full\hierarchical_final_v3_models\level3_family_submodels")
BATCH_SIZE=2048; MAX_EPOCHS=60; LEARNING_RATE=5e-4; WEIGHT_DECAY=1e-4; EARLY_STOPPING_PATIENCE=10; SCHEDULER_PATIENCE=4; SCHEDULER_FACTOR=0.5; MIN_LR=1e-6
...
class SubtypeMLP(nn.Module):
    def __init__(self,input_dim,num_classes):
        super().__init__(); self.net=nn.Sequential(nn.Linear(input_dim,HIDDEN_1),nn.BatchNorm1d(HIDDEN_1),nn.ReLU(),nn.Dropout(DROPOUT_1),nn.Linear(HIDDEN_1,num_classes))
    def forward(self,x): return self.net(x)
def load_split(path):
    df=pd.read_csv(path); feature_cols=[c for c in df.columns if c not in [LABEL_COL,"binary_label","family","family_id","subtype_id","label_id_34"]]; X=df[feature_cols].to_numpy(dtype=np.float32); y=df[TARGET_COL].to_numpy(dtype=np.int64); return df,feature_cols,X,y
...
def main():
    ...
    model=SubtypeMLP(len(feature_cols),len(label_map)).to(device); criterion=nn.CrossEntropyLoss(); optimizer=torch.optim.Adam(model.parameters(),lr=LEARNING_RATE,weight_decay=WEIGHT_DECAY); scheduler=torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer,mode="max",factor=SCHEDULER_FACTOR,patience=SCHEDULER_PATIENCE,min_lr=MIN_LR)
    ...
    cm=confusion_matrix(yt_t,yt_p); pd.DataFrame(cm).to_csv(out_dir/"confusion_matrix_test.csv",index=False); plt.figure(figsize=(8,6)); sns.heatmap(cm,cmap="Blues"); plt.tight_layout(); plt.savefig(out_dir/"confusion_matrix_test.png",dpi=150); plt.close()
    ...
    json.dump({"family":args.family,"best_epoch":best_epoch,"best_val_macro_f1":best_f1,"n_features":len(feature_cols),"training_time_sec":time.time()-start},open(out_dir/"run_summary.json","w"),indent=2)
