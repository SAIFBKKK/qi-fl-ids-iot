"""
train_hierarchical_level2_family.py
==================================

Production trainer for Level 2:
...
SEED = 42
LABEL_COL = "label"
TARGET_COL = "family_id"

DATA_DIR = Path(r"E:\dataset\processed_merged_full\hierarchical_final_v3\level2_family")
OUTPUT_DIR = Path(r"E:\dataset\processed_merged_full\hierarchical_final_v3_models\level2_family")
...
def load_split(path: Path):
    df = pd.read_csv(path)
    feature_cols = [c for c in df.columns if c not in [LABEL_COL, "binary_label", "family", "family_id", "subtype_id", "label_id_34"]]
...
def main():
    ...
    _, feature_cols, X_train, y_train = load_split(DATA_DIR / "train.csv")
    _, _, X_val, y_val = load_split(DATA_DIR / "val.csv")
    _, _, X_test, y_test = load_split(DATA_DIR / "test.csv")
    ...
    model = FamilyMLP(len(feature_cols), len(label_map)).to(device)
    ...
    for epoch in range(1, MAX_EPOCHS + 1):
        ...
        val_m, _, _ = eval_model(model, val_loader, criterion, device)
        ...
        if val_m["macro_f1"] > best_f1 + 1e-5:
    ...
    val_m, _, _ = eval_model(model, val_loader, criterion, device)
    test_m, yt_t, yt_p = eval_model(model, test_loader, criterion, device)

    pd.DataFrame(history).to_csv(OUTPUT_DIR / "training_history.csv", index=False)
    pd.DataFrame(val_m.items(), columns=["metric", "value"]).to_csv(OUTPUT_DIR / "val_metrics.csv", index=False)
    pd.DataFrame(test_m.items(), columns=["metric", "value"]).to_csv(OUTPUT_DIR / "test_metrics.csv", index=False)

    target_names = [id_to_label[i] for i in range(len(id_to_label))]
    pd.DataFrame(
        classification_report(yt_t, yt_p, target_names=target_names, output_dict=True, zero_division=0)
    ).T.to_csv(OUTPUT_DIR / "classification_report_test.csv")

    cm = confusion_matrix(yt_t, yt_p)
    ...
    with open(OUTPUT_DIR / "run_summary.json", "w", encoding="utf-8") as f:
        json.dump({
            ...
            "architecture": "32 -> 64 -> family_count",
        }, f, indent=2)
