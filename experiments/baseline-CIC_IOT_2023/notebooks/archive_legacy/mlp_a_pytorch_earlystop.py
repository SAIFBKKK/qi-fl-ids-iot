...
Why this script
---------------
This version is designed as an upgraded replacement for the earlier sklearn MLP baseline.
It adds:
- PyTorch training
...
Important notes
---------------
- This script is for CENTRALIZED training.
- It is not yet the FL client script.
- It loads the whole CSV, then performs split + scaling.
- BatchNorm and Dropout are active during training only.
- Early stopping uses validation macro-F1 as the main selection criterion.
"""
...
OUTPUT_DIR = Path(r"E:\dataset\processed_merged_full\mlp_a_pytorch_results")
...
# Data split
TEST_SIZE = 0.15
VAL_SIZE_FROM_REMAINING = 0.17647058823529413  # gives 70/15/15 global split
...
class MLPArchitectureA(nn.Module):
...
def main():
    ...
    model = MLPArchitectureA(input_dim=len(feature_cols), num_classes=len(label_to_id)).to(DEVICE)
    ...
    if SAVE_PLOTS:
        save_confusion_matrix(
            ...
            "Confusion Matrix — PyTorch MLP A (32 -> 64 -> 34)"
        )
