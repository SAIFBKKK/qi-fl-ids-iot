"""
stable_flat_mlp_a_pytorch.py
============================

Stable flat PyTorch MLP baseline for CICIoT2023.

Chosen architecture
-------------------
Input(32) -> Linear(64) -> BatchNorm -> ReLU -> Dropout(0.20) -> Output(34)

Main stability fixes
--------------------
- Focal Loss removed
- Weighted CrossEntropy used instead
- Class weights clipped into a safer range
- Lower learning rate
...
OUTPUT_DIR = Path(r"E:\dataset\processed_merged_full\stable_flat_mlp_a_results")
...
class StableFlatMLPA(nn.Module):
...
def main():
    ...
    print("Loss:", "CrossEntropyLoss + clipped class weights", flush=True)
    ...
    X_train, X_temp, y_train, y_temp = train_test_split(
        X, y_encoded, test_size=TEMP_SIZE, random_state=SEED, stratify=y_encoded
    )
    ...
    class_weights = {int(c): float(w) for c, w in zip(classes, clipped_weights)}

    class_weight_df = pd.DataFrame({
        "class_id": list(class_weights.keys()),
        "weight": list(class_weights.values()),
        "label": [id_to_label[i] for i in class_weights.keys()],
        "raw_weight": [float(w) for w in raw_weights],
    }).sort_values("weight", ascending=False)

    show_df("Top class weights (clipped)", class_weight_df, n=20)
    ...
    model = StableFlatMLPA(input_dim=len(feature_cols), num_classes=len(label_to_id)).to(DEVICE)
    ...
    report_dict = classification_report(y_test_true, y_test_pred, target_names=label_encoder.classes_, output_dict=True, zero_division=0)
    ...
    if SAVE_PLOTS:
        save_confusion_matrix(cm, OUTPUT_DIR / "confusion_matrix_test.png", "Confusion Matrix — Stable Flat PyTorch MLP A (32 -> 64 -> 34)")
