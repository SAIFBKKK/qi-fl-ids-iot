"""
train_level1_binary_mlp.py
==========================

Level 1 hierarchical IDS training script.

Task:
- benign vs attack

Reads prepared artifacts from:
E:\dataset\processed_merged_full\hierarchical_prepared\binary

Saves model and metrics to:
E:\dataset\processed_merged_full\hierarchical_models\level1_binary
"""
...
INPUT_DIR = Path(r"E:\dataset\processed_merged_full\hierarchical_prepared\binary")
OUTPUT_DIR = Path(r"E:\dataset\processed_merged_full\hierarchical_models\level1_binary")
...
def main():
    ...
    print("Architecture:", "32 -> 64 -> 2", flush=True)
    ...
    print("Label mapping:", label_mapping, flush=True)
    ...
    show_df("Binary class weights", cw_df, n=10)
    ...
    model = BinaryMLP(input_dim=X_train.shape[1], num_classes=2).to(DEVICE)
    ...
    show_df("Classification report", report_df, n=10)

    print_header("CONFUSION MATRIX")
    cm = confusion_matrix(y_test_true, y_test_pred)
    save_confusion_matrix(cm, OUTPUT_DIR / "confusion_matrix_test.png", "Level 1 Binary Confusion Matrix")
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Macro F1")
    plt.title("Level 1 Binary Validation Macro F1")
    plt.legend()
    plt.tight_layout()
